	...
	function drawImage(x, y, image){
		if(image.complete){
			...
			context.fillRect(x * (CELL_SIZE + col_padding),
	...
	function rightClick(event){
		...
		if(x - (attribute * (CELL_SIZE + col_padding)) < CELL_SIZE &&
			y - category * (CELL_SIZE + row_padding) < CELL_SIZE &&
			attribute != 0 &&
